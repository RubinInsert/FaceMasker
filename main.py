            ...
            for age_range in Age_Ranges:
                ...
                for i in range(1, 11):  # M1.jpg to M10.jpg
                    ...
                    if not os.path.exists(filepath):
                        ...
                        continue
                    for t in range(-6, 7):
                        tinted = colorTint(filepath, (0, 0, t))
                        if tinted is None:
                            continue

                        converted = cv2.cvtColor(tinted, cv2.COLOR_RGB2BGR)
                        # Create output path under "tinted/"
                        output_path = os.path.join("tinted", gender, ethnicity, age_range, "M" + str(i))
                        os.makedirs(output_path, exist_ok=True)
                        cv2.imwrite(os.path.join(output_path, f"Tint - {t}.png"), converted)
